...
export type ResultType<T extends (...args: any) => any> = PromiseReturn<
  ReturnType<T>
>;
...
export const APIHandler = (
  handler:
    | Handler
    | Partial<{ POST: Handler; GET: Handler; PUT: Handler; DELETE: Handler }>
) => {
...
export const multiRouteHandler = (
  query: string,
...
export async function callApi<
  Msg extends object | string | null,
  R extends Omit<Result, "headers">
>(endpoint: string, msg?: Msg, method?: Methods) {
  let result = await fetch(endpoint, {
    method: method ? method : msg ? "POST" : "GET",
    headers: {
      "Content-type":
        typeof msg === "object" ? "application/json" : "text/html",
    },
...
>(
  deps: any[],
  successCallback?: (result: Extract<R, { status: 200 }>["result"]) => any
) {
...
export function useApiData<R extends Omit<Result, "headers">>(
  path?: string,
  initialData?: R
) {
  return useSWR<Success<R>, Errors<R>>(
    path ? path : null,
    async (path) => {
