...
export const usernameValidate = (s: string) =>
  /^[a-zA-Z0-9_.\-]{3,15}$/.test(s);
...
export const sortByDateAndName = (
  a: { start_date: string; name: string },
  b: { start_date: string; name: string }
) => {
  if (a.start_date === b.start_date) return a.name > b.name ? 1 : -1;
  return a.start_date > b.start_date ? 1 : -1;
};

export const cohortName = (name: string) =>
  isNaN(parseInt(name)) ? name : `Cohort #${name}`;
