...
export const useProfileData = (
  username?: string,
  initialData?: Success<ProfileResult>
) => {
  return useSWR(
    username ? "/api/people/" + username : null,
    async (api) => {
...
export const useCourseData = (
  id?: number | string,
  initialData?: Success<CourseDataResult>
) => {
  return useSWR(
...
export const useEventData = (id: number, initialData?: Event) =>
  useSWR(
    `/api/events/${id}`,
    async (api) => {
...
export const useCohortData = (
  cohort?: number,
  initialData?: Success<CohortResult>
) => {
  return useSWR(
    cohort ? `/api/cohorts/${cohort}` : null,
    async (api) => {
...
export const useCourses = (
  options?: Partial<{
    type: "course" | "club";
    initialData?: Success<CourseResult>;
  }>
) => {
  return useSWR(
    `/api/courses${options?.type ? `?type=${options.type}` : ""}`,
    async (api) => {
