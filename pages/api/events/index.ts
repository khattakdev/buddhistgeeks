...
let CreateEventValidator = t.Intersect(
  t.Record({
    start_date: t.String,
    end_date: t.String,
...
export const getPublicEventsQuery = () =>
  ...
  switch (msg.type) {
    case "cohort": {
      let cohort = await prisma.course_cohorts.findUnique({
        ...
        select: {
          cohort_facilitators: {
            include: { people: { select: { username: true } } },
          },
          people_in_cohorts: {
      ...
      if (msg.people) {
        for (let username of msg.people) {
          if (
            !cohort.people_in_cohorts.find(
              (p) => p.people.username === username
            ) &&
            cohort.cohort_facilitators.find(
              (f) => f.people.username === username
            )
          )
            return {
              status: 400,
        ...
        include: {
          events: {
            include: {
              people_in_events: {
                select: {
                  people: { select: { username: true, display_name: true } },
                },
              },
            },
          },
