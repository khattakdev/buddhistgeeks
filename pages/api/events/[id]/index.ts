...
export const eventDataQuery = async (id: number, userId?: string) => {
  let event = await prisma.events.findUnique({
    ...
    include: {
      people: {
        select: { display_name: true, username: true, bio: true, id: true },
      },
      people_in_events: {
        include: {
          people: {
  ...
  if (!event) return;
  let people_in_events = event.people_in_events.map((person) =>
    produce(person, (p) => {
      if (event?.created_by !== userId) p.people.email = "";
    })
  );
  let no_accounts_rsvps = event.no_account_rsvps.map((person) =>
    event?.created_by !== userId ? { ...person, email: "" } : person
  );

  if (
    userId != event.created_by &&
    !event.people_in_events.find((p) => p.person === userId)
  ) {
...
async function getEvent(req: Request) {
  ...
  if (Number.isNaN(eventId))
    return { status: 400, result: "ERROR: event id is not a number" } as const;
  let event = await eventDataQuery(eventId, user?.id);
  if (!event)
    return { status: 404, result: "ERROR: no event with id found" } as const;
...
async function updateEvent(req: Request) {
  ...
  let event = await prisma.events.findUnique({
    where: { id: eventId },
    select: { id: true, created_by: true, no_account_rsvps: true, name: true },
  });
  ...
  switch (msg.type) {
    case "cohort": {
      let cohort = await prisma.course_cohorts.findUnique({
        where: { id: msg.cohort },
        select: { cohort_facilitators: true },
      });
      if (!cohort)
        return {
          status: 404,
      ...
      if (
        ...
        return {
          status: 401,
          result:
            "ERROR: user is not creator of event or a facilitator of the cohort",
        } as const;
      ...
      return {
        status: 200,
        result: { type: "cohort", data: newEvent },
      } as const;
    }
    case "standalone": {
      ...
      if (
        ...
        await sendEventUpdateNoAccountEmail(
          event.no_account_rsvps.map((rsvp) => {
            return {
              email: rsvp.email,
              vars: {
                name: rsvp.name,
      ...
      return {
        status: 200,
        result: { type: "standalone", data: newEvent },
      } as const;
    }
  }
}
...
async function deleteEvent(req: Request) {
        ...
        select: {
          course_cohorts: {
            select: {
              ...
              people_in_cohorts: {
                select: { people: { select: { username: true } } },
              },
            },
          },
        },
  ...
  if (
    !event.cohort_events[0]?.course_cohorts.cohort_facilitators.find(
      (f) => user && f.facilitator === user.id
    ) &&
