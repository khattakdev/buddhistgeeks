...
const WrappedLibraryCollectionsPage = (props: Props) =>
  props.notFound ? h(ErrorPage) : h(LibraryCollections, props);
    ...
    h(
      ...
      props.posts
        ...
        .map((p) =>
          h(
            Link,
            { href: p.path, passHref: true },
            h(Card, { style: { height: "min-content" } }, [
              h(Box, { gap: 16 }, [
                h(Box, { gap: 8 }, [
                  h("h3", p.title),
                  new Date(p.date) >
...
export const getStaticProps = async (ctx: any) => {
  ...
  let posts = fs
    .readdirSync("./pages/library")
    .map((file) => {
      if (fs.lstatSync(path.join("./pages/library/", file)).isDirectory())
        return;
  ...
  return {
    props: { collection, posts: posts as Library[], notFound: false },
  } as const;
};
