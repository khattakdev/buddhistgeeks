...
export const CourseCohortCard = (props: Props) => {
  return h(
    ...
    h(Container, [
      h(ImageContainer, [h("img", { src: props.course.card_image })]),
      h(Content, [
        h(Box, { gap: 4 }, [
        ...
        h("span", [
          ...
          h(ShowSpotsLeft, {
            ...
            people_in_cohorts: props.people_in_cohorts?.length || 0,
    ...
    h(ClubCohortCardContainer, [
      h(ClubCohortCardHeader, [
        h(
          Box,
          { h: true },
          props.course.card_image.split(",").map((src) => h("img", { src }))
        ),
        h(Box, { gap: 4 }, [
          h("h3", props.course.name),
          h("h4.textSecondary", cohortName(props.name)),
...
export const cohortPrettyDate = (
  start_date: string,
  completed?: string | null
) => {
  if (completed)
    return `${prettyDate(start_date)} - ${prettyDate(completed || "")}`;
  if (new Date() > new Date(start_date))
    return `Started ${prettyDate(start_date)}`;
  return `Starts ${prettyDate(start_date)}`;
};

export const ShowSpotsLeft = (props: {
  cohort_max_size: number;
  people_in_cohorts: number;
  start_date: string;
}) => {
  ...
  if (props.cohort_max_size === props.people_in_cohorts)
    return h("span", [
      h("b.textPrimary", " | "),
