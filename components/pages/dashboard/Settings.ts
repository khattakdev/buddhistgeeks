...
const COPY = {
  ...
  displayNameDescription:
    "This is displayed when you post on the forum or enroll in a course.",
  linkField: "Link",
  linkDescription:
    "Where you're hanging on the internet (your website, Twitter, etc.)",
  bioField: "Bio",
};
...
const Settings = (props: {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!changed) return;
    let res = await callUpdatePerson(`/api/people/${props.user.username}`, {
      profile: state,
    });
    if (res.status === 200) {
      props.mutate(state);
  ...
  return h(Box, { gap: 64 }, [
    h(FormBox, { onSubmit, width: 400, gap: 32 }, [
      ...
      h(LabelBox, { gap: 8 }, [h("h4", COPY.bioField), h(Textarea, form.bio)]),
      h(SubmitButtons, [
        h(
          Destructive,
          {
        ...
        h(
          Primary,
          { status, type: "submit", disabled: !changed },
          "Save Changes"
        ),
      ]),
    ]),
...
const StripeSettings = (props: {
  ...
  let onSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setStatus("loading");
    let res = await callApi<null, GETConnectStripeResult>(
      "/api/user/connectStripe?country=" + country
    );
  ...
  return h(FormBox, { width: 400, onSubmit }, [
    h("div", [
      ...
      h(
        "p.textSecondary",
        "We use Stripe to handle payments from learners and payouts to facilitators"
      ),
    ]),
    props.stripe_connected_accounts
      ? null
      : h(Box, [
    ...
    h(
      ...
      !props.stripe_connected_accounts?.connected
        ? "Connect to Stripe"
        : props.stripe_connected_accounts.payouts_enabled
