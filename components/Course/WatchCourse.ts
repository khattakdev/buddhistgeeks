...
import {
  WatchCourseMsg,
  WatchCourseResult,
} from "pages/api/courses/[id]/watch";
...
export function WatchCourse(props: { id: number }) {
  ...
  let watching = userCourses?.watching_courses.find(
    (c) => c.course === props.id
  );
  ...
  return h(WatchCourseBox, [
    ...
    h("div", [
      h("p", [
        watching
          ? "You get emails about new cohorts!"
      ...
      h(
        LinkButton,
        {
          onClick: async (e: React.MouseEvent) => {
            ...
            setLoading(true);
            let res = await callApi<WatchCourseMsg, WatchCourseResult>(
              `/api/courses/${props.id}/watch`,
              { watching: !watching }
            );
            setLoading(false);
            if (res.status === 200) {
              if (watching)
                mutate({
                  ...userCourses,
                  watching_courses: userCourses.watching_courses.filter(
                    (x) => x.course !== props.id
                  ),
                });
              else
                mutate({
                  ...userCourses,
                  watching_courses: [
                    ...userCourses.watching_courses,
                    { course: props.id, email: user.email },
                  ],
                });
            }
          },
...
export function WatchCourseInline(props: { id: number }) {
  ...
  useDebouncedEffect(
    async () => {
      ...
      await callApi<WatchCourseMsg, WatchCourseResult>(
        `/api/courses/${props.id}/watch`,
        { watching: !!watching }
      );
    },
  ...
  return h(Fragment, [
    ...
    h(WatchCourseInlineButton, { onClick }, [
      ...
      h("small.textSecondary", [
        watching
          ? "You get updates about this course!"
          : h("span", [h("u", "Get updates"), " on new cohorts"]),
      ]),
    ]),
  ]);
}
  ...
  return h(WatchCourseBox, [
    ...
    h(Box, { gap: 8 }, [
      h("b", [
        watching
          ? "You get emails about new cohorts!"
          : "Email me when new cohorts are scheduled",
      ]),
      watching
        ? null
        : h(FormBox, { gap: 8, onSubmit }, [
            h(Input, {
              value: email,
              type: "email",
              placeholder: "your email",
            ...
            h(
              LinkButton,
              { type: "submit", style: { justifySelf: "right" } },
              loading ? h(Loader) : "subscribe"
            ),
          ]),
    ]),
