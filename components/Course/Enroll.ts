...
const Enroll: React.FC<Props> = (props) => {
  ...
  useEffect(() => {
    (async function () {
      ...
      let res = await callApi<null, GetDiscountResult>(
        "/api/discounts/" + discount.code
      );
      if (
        res.status !== 200 ||
        res.result.deleted ||
  ...
  return h(Box, { gap: 16 }, [
    ...
    h(Box, { gap: 8, style: { color: colors.textSecondary } }, [
      !props.cohort_start_date
        ? null
        : h("b", `${prettyDate(props.cohort_start_date)}`),
      h("b", props.course?.duration),
      props.course?.cohort_max_size === 0
        ? null
        : h("b", `Up to ${props.course?.cohort_max_size} learners`),
      h(Box, { gap: 4 }, [
        h("b", "Prerequisites"),
        h(Text, {
          source: props.course?.prerequisites,
          disallowedTypes: ["heading" as const],
        }),
      ]),
    ]),
